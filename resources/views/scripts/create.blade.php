    ...
    <div class="bg-white rounded-lg shadow-lg">
        <div class="px-6 py-4 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800">{{ $title ?? 'Nouveau Script' }}</h2>
        ...
        <div class="p-6">
            <p class="text-gray-600 mb-6">{{ $message ?? 'Créer un nouveau script' }}</p>

            <form method="POST" action="{{ route('scripts.store') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" 
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Description du script"></textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-4">
                    ...
                    <button type="submit" 
                        ...
                        <i class="fas fa-save mr-2"></i>Créer le script
                    </button>
                </div>
            </form>
        </div>
